...
try {
    ...
    Write-Host ""
    Write-Host "  3. Para cada artefato, o papel humano correspondente abre uma"
    Write-Host "     sessão de assistente de IA (ex.: 'claude' para Claude Code)"
    Write-Host "     e supervisiona a produção. A ordem do fluxo de quatro etapas"
    Write-Host "     é inviolável; antes de qualquer linha de código, produza:"
    Write-Host ""
    Write-Host "       docs/GOAL.md          <- etapa 1 — objetivo (engenheiro)"
    Write-Host "       docs/BUSINESS.md      <- etapa 2 — problema (analista)"
    Write-Host "       docs/GUIDELINE.md     ┐"
    Write-Host "       docs/ARCHITECTURE.md  ├ etapa 3 (paralela) — projeção (designer + arquiteto)"
    Write-Host "       docs/SOLUTION.md      ┘"
    Write-Host "                             -> etapa 4 — implementação em src/ e test/ (engenheiro)"
    Write-Host ""
} finally {
