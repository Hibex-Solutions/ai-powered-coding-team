...
Esta seção documenta as decisões de stack tecnológica para projetos baseados em .NET.
...
Todo projeto .NET gerado a partir deste template deve ter os seguintes arquivos na raiz:
...
Use `/dotnet-engineer` no Claude Code para delegar a implementação ao engenheiro .NET especializado.
